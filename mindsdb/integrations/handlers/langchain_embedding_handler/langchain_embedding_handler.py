...
def get_langchain_class(class_name: str) -> Embeddings:
    ...
    try:
        module = importlib.import_module("langchain_community.embeddings")
        class_ = getattr(module, class_name)
        # Skip if the class is a broken Pydantic stub (incomplete forward references)
        if getattr(class_, "__pydantic_complete__", True):
            return class_
    except ImportError:
        raise Exception(
    ...
    # Fallback: try dedicated provider packages (e.g. langchain_google_vertexai, langchain_openai)
    provider_modules = [
    ...
    for mod_name in provider_modules:
        try:
            ...
            if hasattr(mod, class_name):
                ...
                # Some Pydantic v2 models have unresolved forward references and need model_rebuild()
                if hasattr(class_, "model_rebuild"):
                    try:
    ...
    raise Exception(
        f"Could not find the class {class_name} in langchain_community.embeddings or any provider package. "
        f"Please install the relevant package (e.g. pip install langchain-google-vertexai)."
    )
