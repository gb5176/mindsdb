...
class TablesCollection:
    ...
    def match(self, table: Identifier) -> bool:
        ...
        if db is None:
            ...
            # Try matching against all known databases (LLM may omit database prefix)
            for known_db in self.databases:
                if known_db is not None:
                    if self.match(Identifier(parts=[known_db, tbl])):
                        return True
            return False
        ...
        if any([fnmatch.fnmatch(tbl, pattern) for pattern in self._dbs[db]]):
            return True

        # db might actually be a schema name — LLM used schema.table instead of database.schema.table
        # Check if any known database has this as a schema
        for known_db in self.databases:
            if known_db is not None and known_db != db:
                if schema is not None:
                    # 3-part name that didn't match — no further fallback
                    continue
                # Treat db as schema, check if known_db.db has tbl
                if any([fnmatch.fnmatch(tbl, pattern) for pattern in self._schemas.get(known_db, {}).get(db, [])]):
                    return True
