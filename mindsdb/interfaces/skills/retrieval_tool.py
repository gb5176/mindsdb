...
def _load_rag_config(tool: dict, pred_args: dict, skill: db.Skills) -> RAGPipelineModel:
    ...
    if "source" in tool:
        ...
        if kb_table._kb.embedding_model:
            # Old-style: embedding model created via CREATE MODEL
            embedding_args = kb_table._kb.embedding_model.learn_args.get("using", {})
            embeddings_model = construct_model_from_args(embedding_args)
            logger.debug(f"Using knowledge base embedding model with args: {embedding_args}")
        elif kb_table._kb.params.get("embedding_model"):
            # New-style: inline embedding_model JSON params in CREATE KNOWLEDGE BASE ... USING
            from mindsdb.interfaces.knowledge_base.controller import adapt_embedding_model_params, get_model_params
            embedding_model_params = get_model_params(kb_table._kb.params.get("embedding_model", {}), "default_embedding_model")
            embeddings_model = construct_model_from_args(adapt_embedding_model_params(embedding_model_params))
            logger.debug(f"Using knowledge base inline embedding model params: {kb_table._kb.params['embedding_model']}")
        else:
