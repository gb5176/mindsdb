...
DEFAULT_AGENT_TIMEOUT_SECONDS = 1500
...
1. For questions about database tables and their contents, follow this sequence to minimise token usage:

   **Step 1 — Identify relevant tables using a table catalog tool (if available):**
   - Look at your available tools list for a tool whose description mentions "table catalog" or "available database tables"
   - If found, call that tool with the user's question to retrieve the relevant table names
   - If no such tool exists, fall back to sql_db_list_tables

   **Step 2 — Identify relevant columns using a column catalog tool (if available):**
   - Look at your available tools list for a tool whose description mentions "column catalog" or "columns of the tables"
   - If found, call that tool with the user's question to retrieve which specific columns are relevant
   - The column catalog tool will return column names and their business meaning

   **Step 3 — Schema loading (only if needed):**
   - If Step 2 gave you sufficient column information, SKIP sql_db_schema entirely and write the SQL query directly
   - Only call sql_db_schema if the catalog tools did not provide enough column detail, and only for the specific tables identified in Step 1

   **Step 4 — Query execution:**
   - Write a SQL query using only the columns identified from the catalog tools or schema
   - Always include join/key columns (columns ending in _SKEY, _ID, _KEY) needed for table joins even if not explicitly returned by catalog tools
   - Use sql_db_query to execute the query and return results to the user

2. For factual questions about specific topics, look for any retrieval or knowledge base tools in your tool list and call them with the user's question.
...
1. For questions about database tables and their contents, follow this sequence to minimise token usage:
   ...
   **Step 4 — Query execution:**
   ...
   - Use sql_db_query to execute the query. The SQL query string itself is NOT the final answer — your response must be based on the results returned by sql_db_query
...
**IMPORTANT: You MUST stop after "Action Input:". Do NOT generate "Observation:" or predict tool results. The system will execute the tool and provide the real observation. Never fabricate or guess tool outputs.**
