...
class SafeOutputParser(AgentOutputParser):
    ...
    def parse(self, text: str) -> Union[AgentAction, AgentFinish]:
        ...
        # LLMs (especially Gemini) sometimes hallucinate the entire ReAct loop
        # (Action + fake Observation + next Action + fake Observation + final AI answer)
        # in a single response. Observations should only come from tool execution,
        # never from the LLM itself. Cutting here forces the agent to actually
        # execute the tool and use real results.
