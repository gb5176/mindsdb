...
class ChatMindsdb(BaseChatModel):
    ...
    def completion(
        ...
        # Extract token usage if the handler populated the hidden columns
        usage = {}
        if '__prompt_tokens__' in predictions.columns:
            usage = {
                'prompt_tokens': int(predictions['__prompt_tokens__'][0] or 0),
                'completion_tokens': int(predictions['__completion_tokens__'][0] or 0),
                'total_tokens': int(predictions['__total_tokens__'][0] or 0),
            }
