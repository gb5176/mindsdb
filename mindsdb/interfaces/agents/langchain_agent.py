...
# ----- Monkey-patch: Fix token usage extraction for Gemini 2.5 models -----
# langchain-google-genai v2.1.9 fails on response.usage_metadata.thoughts_token_count
# (AttributeError) which discards ALL token usage data. This patch recovers it.
import langchain_google_genai.chat_models as _gcm

_orig_response_to_result = _gcm._response_to_result


def _patched_response_to_result(response, stream=False, prev_usage=None):
    """Patched _response_to_result that handles missing thoughts_token_count."""
    # Extract raw token counts before the original function (which may lose them)
    raw_input = raw_output = raw_total = 0
    try:
        usage = response.usage_metadata
        if usage is not None:
            raw_input = getattr(usage, 'prompt_token_count', 0) or 0
            raw_output = getattr(usage, 'candidates_token_count', 0) or 0
            # total_token_count may be 0 in proto3 when unset; fall back to sum
            raw_total = getattr(usage, 'total_token_count', 0) or (raw_input + raw_output)
    except Exception:
        pass

    # Call the original conversion
    result = _orig_response_to_result(response, stream=stream, prev_usage=prev_usage)

    # If original lost token data (due to thoughts_token_count AttributeError), restore it.
    # Check for None OR zero-valued UsageMetadata (original may zero-out on AttributeError).
    if raw_total > 0:
        for gen in result.generations:
            if hasattr(gen, 'message'):
                ...
                if um is None:
                    ...
                    current_total = um.get('total_tokens', 0) or 0
                else:
                    current_total = getattr(um, 'total_tokens', 0) or 0
                if current_total == 0:
                    from langchain_core.messages.ai import UsageMetadata
                    gen.message.usage_metadata = UsageMetadata(
...
_gcm._response_to_result = _patched_response_to_result
# ----- End monkey-patch -----
...
from .callback_handlers import LogCallbackHandler, ContextCaptureCallback, TokenTrackingCallbackHandler
...
def create_chat_model(args: Dict):
    ...
    if args["provider"] == "google":
        # Add stop sequence to prevent Gemini from hallucinating tool observations.
        # Gemini tends to generate the entire ReAct loop (Action + fake Observation + next Action...)
        # in a single response, wasting tokens and producing fabricated results.
        model_kwargs.setdefault('stop', []).append('\nObservation:')
...
class LangchainAgent:
    ...
    def _get_agent_callbacks(self, args: Dict) -> List:
        ...
        # Token tracking callback - tracks LLM calls, token usage, and tool invocations
        self.token_tracking_handler = TokenTrackingCallbackHandler(logger)
        all_callbacks.append(self.token_tracking_handler)
...
AI: {response}"""
    ...
    def run_agent(self, df: pd.DataFrame, agent: AgentExecutor, args: Dict) -> pd.DataFrame:
        ...
        # Log token tracking summary if available
        if hasattr(self, 'token_tracking_handler') and self.token_tracking_handler is not None:
            summary = self.token_tracking_handler.get_summary()
            logger.info(
